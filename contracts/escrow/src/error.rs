...
/// Internal CCTP validation errors — mapped to `EscrowError` before they
/// reach any contract entrypoint.
#[derive(Debug, Copy, Clone, PartialEq)]
#[contracterror]
pub enum CctpError {
    ...
    /// route's amount — defense-in-depth against a bogus/compromised value
    /// from a caller bypassing the API's live Circle quote.
    MaxFeeExceedsCap = 3,
}

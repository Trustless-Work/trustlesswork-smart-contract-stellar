...
fn test_batch_release_partial_then_full() {
    ...
    assert!(
        result.is_err(),
        "Re-releasing an already released milestone must fail"
    );
...
fn test_withdraw_remaining_funds_rounding_edge_case() {
    ...
    client.dispute_milestones(
        &approver,
        &vec![&env, 0u32],
        &String::from_str(&env, "Work disputed"),
    );
...
fn test_withdraw_remaining_funds_rejects_partial_withdrawal() {
    ...
    let mut resolve_dist = Map::new(&env);
    resolve_dist.set(service_provider.clone(), escrow_amount);
    client.resolve_dispute(
        &dispute_resolver,
        &trustless_work_address,
        &vec![&env, 0u32],
...
#[test]
fn test_release_funds_rejects_oversized_indices_vector() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let escrow_properties = Escrow {
        engagement_id: String::from_str(&env, "oversized_indices"),
        title: String::from_str(&env, "Test"),
        description: String::from_str(&env, "Desc"),
        roles: Roles {
            approvers: vec![&env, approver.clone()],
    ...
    // Escrow has 1 milestone; a release request with more indices than that is
    // rejected up front (before the O(n^2) duplicate scan).
    let oversized = vec![&env, 0u32, 0u32, 0u32];
    let result = client.try_release_funds(&release_signer, &trustless_work, &oversized);
    assert_eq!(
        result.err(),
        Some(Ok(crate::error::ReleaseError::BatchTooLarge))
    );
}
...
fn test_fund_escrow_accumulation_overflow_is_controlled() {
    ...
    env.as_contract(&client.address, || {
        env.storage()
            .persistent()
            .set(&crate::storage::types::DataKey::FundedAmount, &i128::MAX);
    });

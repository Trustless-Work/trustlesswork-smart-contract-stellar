...
#[test]
fn test_withdraw_remaining_funds_reentrancy_guard() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Simulate an in-flight call (as if a malicious token reentered): the
    // reentrancy flag is already set. withdraw_remaining_funds must reject
    // immediately, with the dedicated Reentrancy error (not FlagsMustBeFalse).
    env.as_contract(&client.address, || {
        env.storage()
            .persistent()
            .set(&crate::storage::types::DataKey::Reentrancy, &true);
    ...
    distributions.set(service_provider.clone(), 100_000_000i128);
    let result =
        client.try_withdraw_remaining_funds(&dispute_resolver, &trustless_work, &distributions);
    assert_eq!(
        result.err(),
        Some(Ok(crate::error::EscrowError::Reentrancy))
    );
